...
If you want to use it on an embedded system, you need to use your own `GlobalAlloc`. The original repo (https://github.com/RustAudio/sample) is using core::intrinsics, however, then a C implementation of libm needs to be linked (https://github.com/rust-lang/rfcs/issues/2505). The best part is that it's now a libm Rust-lib which is used here (https://github.com/rust-lang-nursery/libm). 
